...
class MIoTMediaDecoder(threading.Thread):
    ...
    def choose_hw_decoder(self, codec_name, hw_methods):
        ...
        if codec_name in ("h264", "hevc"):
            # 优先 NVIDIA CUVID（需要 ffmpeg 编译时带 --enable-cuvid）
            if "cuda" in hw_methods or "cuvid" in hw_methods:
                cuvid = f"{codec_name}_cuvid"
                if try_codec(cuvid):
                    _LOGGER.info("使用 NVIDIA GPU 解码器: %s", cuvid)
                    return cuvid
            # 次选 V4L2（ARM/嵌入式设备）
            v4l2 = f"{codec_name}_v4l2m2m"
            if v4l2 in hw_methods or try_codec(v4l2):
                _LOGGER.info("使用 V4L2 硬件解码器: %s", v4l2)
                return v4l2

        _LOGGER.info("使用 CPU 软件解码器: %s", codec_name)
        return codec_name

    def _on_video_callback(self, frame_data: MIoTCameraFrameData) -> None:
        if not self._video_decoder:
            codec_name = "h264" if frame_data.codec_id == MIoTCameraCodec.VIDEO_H264 else "hevc"
            if self._enable_hw_accel:
                hw_methods = self.detect_hwaccel()
                hw_codec = self.choose_hw_decoder(codec_name, hw_methods)
                try:
                    self._video_decoder = VideoCodecContext.create(hw_codec, "r")
                    _LOGGER.info("video hw decoder created, %s", hw_codec)
                except Exception:
                    _LOGGER.warning("hw decoder %s failed, fallback to software", hw_codec)
                    self._video_decoder = VideoCodecContext.create(codec_name, "r")
            else:
                self._video_decoder = VideoCodecContext.create(codec_name, "r")
