...
def calc_group_id(uid: str, home_id: str) -> str:
    ...
    return hashlib.sha1(f"{uid}central_service{home_id}".encode("utf-8")).hexdigest()[:16]
...
async def http_get_async(
    url: str, params: Optional[Dict] = None, headers: Optional[Dict] = None, loop: Optional[AbstractEventLoop] = None
) -> str:
    """Http get."""
    full_url = url
    if params:
    ...
    async with ClientSession(loop=loop or asyncio.get_running_loop()) as session:
        async with session.get(url=full_url, headers=headers or {}, timeout=ClientTimeout(total=30)) as response:
...
async def http_post_json_async(
    url: str, data: Dict, headers: Optional[Dict] = None, loop: Optional[AbstractEventLoop] = None
) -> Dict:
    """Http post json."""
    async with ClientSession(loop=loop) as session:
