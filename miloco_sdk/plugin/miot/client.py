...
from miloco_sdk.plugin.miot.camera import MIoTCamera, MIoTCameraInstance, get_camera_extra_info
from miloco_sdk.plugin.miot.cloud import MIoTHttpClient, MIoTOAuth2Client
from miloco_sdk.plugin.miot.lan import MIoTLan
from miloco_sdk.plugin.miot.network import MIoTNetwork
from miloco_sdk.utils.const import CLOUD_SERVER_DEFAULT, SYSTEM_LANGUAGE_DEFAULT
...
class MIoTClient:
    ...
    async def __on_lan_device_status_changed(self, did: str, info: MIoTLanDeviceInfo, ctx: Any = None) -> None:
    ...
    async def init_async(self) -> None:
        ...
        self._oauth_client = MIoTOAuth2Client(
            redirect_uri=self._redirect_uri, cloud_server=self._cloud_server, uuid=self._uuid, loop=self._main_loop
        )
    ...
    async def create_camera_instance_async(
        self, camera_info: MIoTCameraInfo, frame_interval: int = 500, enable_hw_accel: bool = True
    ) -> MIoTCameraInstance:
        ...
        return await self._camera_client.create_camera_async(
            camera_info=camera_info, frame_interval=frame_interval, enable_hw_accel=enable_hw_accel
        )
