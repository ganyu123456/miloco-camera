...
from miloco_sdk.plugin.miot.network import MIoTNetwork
from miloco_sdk.utils.types import InterfaceStatus, MIoTLanDeviceInfo, NetworkInfo
...
class _MIoTLanDevice:
    ...
    def __broadcast_info_changed(self):
        self._manager.broadcast_device_info_changed(
            did=self.did, info=MIoTLanDeviceInfo(did=self.did, online=self._online, ip=self._ip)
        )
...
class MIoTLan:
    ...
    async def register_status_changed_async(
        self, key: str, handler: Callable[[str, MIoTLanDeviceInfo, Any], Coroutine], handler_ctx: Any = None
    ) -> bool:
        """Register status changed."""
        if not self._init_done:
            return False
        self._internal_loop.call_soon_threadsafe(
            self.__register_status_changed, _MIoTLanRegDeviceData(key=key, handler=handler, handler_ctx=handler_ctx)
        )
        return True
    ...
    async def ping_async(self, if_name: Optional[str] = None, target_ip: Optional[str] = None) -> None:
        ...
        fut = asyncio.run_coroutine_threadsafe(
            coro=asyncio.to_thread(self.ping_internal, if_name, target_ip), loop=self._internal_loop
        )
    ...
    def ping_internal(self, if_name: Optional[str] = None, target_ip: Optional[str] = None) -> None:
        ...
        self.__sendto(if_name=if_name, data=self._probe_msg, address=target_ip or "255.255.255.255", port=self.OT_PORT)
    ...
    def broadcast_device_info_changed(self, did: str, info: MIoTLanDeviceInfo) -> None:
        ...
        for handler in self._callbacks_device_status_changed.values():
            self._main_loop.call_soon_threadsafe(
                self._main_loop.create_task, handler.handler(did, info, handler.handler_ctx)
            )
    ...
    def __create_socket(self, if_name: str) -> None:
        ...
        try:
            ...
            # Set SO_BINDTODEVICE（仅在支持该选项的平台上，例如 Linux）
            if hasattr(socket, "SO_BINDTODEVICE"):
                try:
                    sock.setsockopt(socket.SOL_SOCKET, socket.SO_BINDTODEVICE, if_name.encode())
                except Exception as bind_err:  # pylint: disable=broad-exception-caught
                    _LOGGER.warning("set SO_BINDTODEVICE failed, %s, %s", if_name, bind_err)
            ...
            # 在 macOS 上，由于没有 SO_BINDTODEVICE，多个接口可能会竞争同一个端口，导致 [Errno 48] Address already in use。
            # 这种情况不影响已建立的 socket 工作，这里仅降级为调试日志以避免大量噪音。
            if getattr(err, "errno", None) == 48:
                _LOGGER.debug("socket already bound on another iface, ignore, %s, %s", if_name, err)
            else:
                _LOGGER.error("create socket os error, %s, %s", if_name, err)
    ...
    def __raw_message_handler(self, data: bytearray, data_len: int, ip: str, if_name: str) -> None:
    ...
    def __sendto(self, if_name: Optional[str], data: bytes, address: str, port: int) -> None:
        if if_name is None:
            # Broadcast
    ...
    async def __on_network_info_change_external_async(self, status: InterfaceStatus, info: NetworkInfo) -> None:
        """Network info change."""
        ...
        if not self._init_done:
            ...
            return
        self._internal_loop.call_soon_threadsafe(
            self.__on_network_info_change, _MIoTLanNetworkUpdateData(status=status, if_name=info.name)
        )
    ...
    async def __get_devices_internal_async(self) -> Dict[str, MIoTLanDeviceInfo]:
        ...
        for did, lan_device in self._lan_devices.items():
            devices[did] = MIoTLanDeviceInfo(did=lan_device.did, online=lan_device.online, ip=lan_device.ip)
        return devices
