...
from miloco_sdk.utils.common import calc_group_id
...
from miloco_sdk.utils.error import MIoTErrorCode, MIoTHttpError, MIoTOAuth2Error
...
class MIoTOAuth2Client:
    ...
    async def __get_token_async(self, data) -> MIoTOauthInfo:
        ...
        if http_res.status == 401:
            _LOGGER.error("unauthorized(401), oauth/get_token, %s -> %s", data, await http_res.text(encoding="utf-8"))
...
class MIoTHttpClient:
    ...
    def __init__(self, cloud_server: str, access_token: str, loop: Optional[asyncio.AbstractEventLoop] = None) -> None:
        """Initialize."""
        self._main_loop = loop or asyncio.get_running_loop()
    ...
    def update_http_header(self, cloud_server: Optional[str] = None, access_token: Optional[str] = None) -> None:
        """Update http header."""
    ...
    async def __mihome_api_get_async(self, url_path: str, params: Dict, timeout: int = MIHOME_HTTP_API_TIMEOUT) -> Dict:
        """Get data from mihome api with http get."""
        # pylint: disable=unused-private-member
        http_res = await self._session.get(
            url=f"{self._base_url}{url_path}", params=params, headers=self.__api_request_headers, timeout=timeout
        )
        if http_res.status == 401:
            _LOGGER.error(
    ...
    async def __mihome_api_post_async(self, url_path: str, data: Dict, timeout: int = MIHOME_HTTP_API_TIMEOUT) -> Dict:
    ...
    async def __get_dev_room_page_async(self, max_id: Optional[str] = None) -> Dict:
        ...
        if res_obj["result"].get("has_more", False) and isinstance(res_obj["result"].get("max_id", None), str):
            ...
            for home_id, home_info in more_list.items():
                ...
                for room_id, room_info in home_info.get("room_list", {}).items():
                    home_infos[home_id].room_list.setdefault(
                        room_id,
                        MIoTRoomInfo(room_id=room_id, room_name="", create_ts=room_info.get("create_time", 0), dids=[]),
                    )
    ...
    async def __get_urn_by_model_async(self, model: str, version: int = 0) -> Optional[str]:
        """Get urn by model."""
        http_res = await self._session.get(
            url="https://miot-spec.org/internal/urn-by-model-version", params={"model": model, "version": 0}, timeout=10
        )
        if http_res.status != 200:
    ...
    async def __get_device_list_page_async(
        ...
        res_obj = await self.__mihome_api_post_async(url_path="/app/v2/home/device_list_page", data=req_data)
        if "result" not in res_obj:
            raise MIoTHttpError(f"invalid response result, {res_obj}")
        ...
        if res_obj.get("has_more", False) and next_start_did:
            device_infos.update(await self.__get_device_list_page_async(dids=dids, start_did=next_start_did))
        return device_infos

    async def get_devices_with_dids_async(self, dids: List[str]) -> Optional[Dict[str, MIoTDeviceInfo]]:
    ...
    async def __get_prop_handler(self) -> bool:
        ...
        if self._get_prop_list:
            self._get_prop_timer = self._main_loop.call_later(
                self._GET_PROP_AGGREGATE_INTERVAL, lambda: self._main_loop.create_task(self.__get_prop_handler())
            )
        else:
            self._get_prop_timer = None
        return True

    async def get_prop_async(self, param: MIoTGetPropertyParam, immediately: bool = False) -> Any:
        """Get prop."""
        if immediately:
    ...
    async def set_prop_async(self, param: MIoTSetPropertyParam) -> Dict:
        """Set prop."""
        res_obj = await self.__mihome_api_post_async(
            url_path="/app/v2/miotspec/prop/set", data={"params": [param.model_dump()]}, timeout=15
        )
    ...
    async def set_props_async(self, params: List[MIoTSetPropertyParam]) -> List:
        ...
        """
        res_obj = await self.__mihome_api_post_async(
            url_path="/app/v2/miotspec/prop/set", data={"params": [param.model_dump() for param in params]}, timeout=15
        )
        if "result" not in res_obj:
            raise MIoTHttpError("invalid response result")
    ...
    async def action_async(self, param: MIoTActionParam) -> Dict:
        ...
        res_obj = await self.__mihome_api_post_async(
            url_path="/app/v2/miotspec/action", data={"params": param.model_dump(by_alias=True)}, timeout=15
        )
        if "result" not in res_obj:
            raise MIoTHttpError("invalid response result")
    ...
    async def __get_manual_scenes_with_home_id_async(
        ...
        req_data: Dict = {"home_id": home_id, "owner_uid": uid, "source": "zkp", "get_type": 2}
        if isinstance(scene_ids, List) and scene_ids:
            req_data["scene_ids"] = scene_ids
        res_obj = await self.__mihome_api_post_async(
            url_path="/app/appgateway/miot/appsceneservice/AppSceneService/GetManualSceneList", data=req_data
        )
        if "result" not in res_obj:
            raise MIoTHttpError(f"invalid response result, {res_obj}")
    ...
    async def run_manual_scene_async(self, scene_info: MIoTManualSceneInfo) -> bool:
        """Run manual scene."""
        req_data: Dict = {"owner_uid": scene_info.uid, "scene_id": scene_info.scene_id, "scene_type": 2}
        ...
        res_obj = await self.__mihome_api_post_async(
            url_path="/app/appgateway/miot/appsceneservice/AppSceneService/NewRunScene", data=req_data
        )
        if "result" not in res_obj:
            raise MIoTHttpError(f"invalid response result, {res_obj}")
    ...
    async def send_app_notify_async(self, notify_id: str) -> bool:
        """Send app notify."""
        res_obj = await self.__mihome_api_post_async(url_path="/app/v2/oauth/send_push", data={"key": notify_id})
        if "result" not in res_obj:
            raise MIoTHttpError(f"invalid response result, {res_obj}")
    ...
    async def create_app_notify_async(self, text: str) -> str:
        ...
        res_obj = await self.__mihome_api_post_async(url_path="/app/v2/oauth/save_text", data={"text": text})
        if "result" not in res_obj and isinstance(res_obj["result"], str):
            raise MIoTHttpError(f"invalid response result, {res_obj}")
        return res_obj["result"]

    async def get_app_notifies_async(self, notify_ids: str | List[str] | None = None) -> Dict[str, MIoTAppNotify]:
        ...
        if notify_ids is None:
            ...
            keys = notify_ids
        res_obj = await self.__mihome_api_post_async(url_path="/app/v2/oauth/get_texts", data={"keys": keys})
